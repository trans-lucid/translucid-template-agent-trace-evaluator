...
You are joining an AI-agent startup. The support agent often gives plausible final answers, but its path can still be unsafe: wrong tools, unsupported claims, repeated failed tool loops, confident answers when escalation is required, and hidden cost/latency regressions.
